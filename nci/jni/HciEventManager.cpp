...
#include "SecureElement.h" // MODIFIED by zhangjie, 2020-12-14,BUG-10277814
...
std::vector<uint8_t> HciEventManager::getDataFromBerTlv(
  ...
  if (lengthTag < 0x80 && berTlv.size() == (lengthTag + 1)) {
    ...
    size_t length = (size_t)((berTlv[1] << 24) | (berTlv[2] << 16) | (berTlv[3] << 8) | berTlv[4]); // MODIFIED by zhangjie, 2020-12-14,BUG-10277814
